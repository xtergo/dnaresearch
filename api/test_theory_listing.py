...
def test_list_theories_combined_filters():
    ...
    response = client.get("/theories?scope=autism&lifecycle=active&sort_by=evidence_count")
    assert response.status_code == 200
...
def test_theory_listing_response_structure():
    ...
    required_fields = [
        "id", "version", "title", "scope", "lifecycle", "author",
        "created_at", "updated_at", "evidence_count", "posterior",
        "support_class", "tags", "has_comments"
    ]
